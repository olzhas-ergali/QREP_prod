...
from service.API.infrastructure.database.models import RegTemp, Client, User
...
async def authorization_handler(
    ...
    # + NEW
    if isinstance(user, User) and user.profile and user.profile.is_active:
        return await staff_main.start_handler(
            message=message,
            user=user,
            state=state
        )
    await staff_auth.auth_phone_handler(message, state)
