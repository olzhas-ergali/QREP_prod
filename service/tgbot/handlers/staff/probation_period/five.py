...
from service.API.infrastructure.database.probation_period import ProbationPeriodAnswer
...
async def probation_period_five_day_handler(
    ...
    if current_question.get('files'):
        ...
        await c.message.answer_document(
            ...
            document=InputFile(current_question.get('files').get(user.profile.local),
                               current_question.get('files').get(user.profile.local).name)
        )
