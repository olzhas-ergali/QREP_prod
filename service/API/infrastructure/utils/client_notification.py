...
async def send_notification_email(
    ...
    # 1. Сначала берем язык из профиля клиента (если он есть)
    local = client.local
    
    # 2. Если в профиле пусто, пробуем узнать через SendPlus
    if not local:
        local = await wb.get_local_by_phone(client.phone_number)
        
    logging.info(f"Email language for {client.phone_number}: {local}")
